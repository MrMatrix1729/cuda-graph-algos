...
__global__ void prims(int *row_ptr, int *col_ind, int *weights, int *mst_edges, int *visited, int num_nodes) {
...
void computeMST(int *h_row_ptr, int *h_col_ind, int *h_weights, int *h_adj_list, int *h_offset, int num_nodes) {
    ...
    cudaMemcpy(d_visited, h_visited, num_nodes * sizeof(int), cudaMemcpyHostToDevice);
        ...
        cudaMalloc(&d_weights, h_row_ptr[num_nodes] * sizeof(int));
        ...
        cudaMemcpy(d_weights, h_weights, h_row_ptr[num_nodes] * sizeof(int), cudaMemcpyHostToDevice);
        
        prims<<<(num_nodes + 255) / 256, 256>>>(d_row_ptr, d_col_ind, d_weights, d_mst_edges, d_visited, num_nodes);
...
int main() {
    ...
    computeMST(h_row_ptr, h_col_ind, h_weights, h_adj_list, h_offset, num_nodes);
